...
Use this guide when setting up `api/stamp` after cloning the project.

## Ownership

`api/stamp` uses the shared database at runtime, but it does not own migrations.
...
```sql
CREATE ROLE gracon_stamp_app LOGIN PASSWORD 'replace_with_stamp_password';
```

If it already exists:

```sql
ALTER ROLE gracon_stamp_app WITH PASSWORD 'replace_with_stamp_password';
```
...
```env
DATABASE_URL=postgresql://gracon_stamp_app:password@ep-example-pooler.us-east-1.aws.neon.tech/neondb?sslmode=require
```
